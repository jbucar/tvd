...
#ifdef _WIN32
//	VisualStudio 2010 seems to have problems with the previous definition
		...
		{			/* Probably :-) 32-bpp */
			...
			for (y = y1; y <= y2; y++) {
				...
				for (x = x1; x <= x2; x++) {
					...
					Uint32 dest_a = (*pixel & Amask) >> Ashift;
					Uint32 source_a = alpha;
					Uint32 out_a = source_a + ((dest_a * (0xFF-source_a))>> 8 ) ;
					A = (out_a << Ashift) & Amask;
					
					if ( out_a )  {
						Uint32 r_dest = (*pixel & Rmask) >> Rshift;
						Uint32 r_source = dR >> Rshift;
						Uint32 r_res = r_source*source_a + ((r_dest*dest_a*(0xFF-source_a)) >> 8 );
						Uint32 out_r = r_res / out_a;

						Uint32 g_dest = (*pixel & Gmask) >> Gshift;
						Uint32 g_source = dG >> Gshift;
						Uint32 g_res = g_source*source_a + ((g_dest*dest_a*(0xFF-source_a)) >> 8);
						Uint32 out_g = g_res / out_a;

						Uint32 b_dest = (*pixel & Bmask) >> Bshift;
						Uint32 b_source = dB >> Bshift;
						Uint32 b_res = b_source*source_a + ((b_dest*dest_a*(0xFF-source_a)) >> 8);
						Uint32 out_b = b_res / out_a;

						R = (out_r << Rshift) & Rmask;
						G = (out_g << Gshift) & Gmask;
						B = (out_b << Bshift) & Bmask;
					} else {
						R = 0;
						G = 0;
						B = 0;
