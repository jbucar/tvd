...
namespace model {
namespace event {
AttributionEvent::AttributionEvent( const std::string &id, components::ExecutionObject* eObject, PropertyAnchor* anchor, ncl30presenter::event::EventManager *manager ) :
...
string AttributionEvent::getCurrentValue() {
	...
	if (settingNode) {
		...
		if (propName != "") {
			maintainerValue =  player::settings::getProperty( propName );
		}
	} else {
