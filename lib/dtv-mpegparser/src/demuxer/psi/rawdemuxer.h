...
class RawDemuxer : public PSIDemuxer {
public:
	explicit RawDemuxer( ID pid );
	RawDemuxer( ID pid, util::DWORD timeout, bool std );	
	virtual ~RawDemuxer();

	void setFilter( util::BYTE tID, util::BYTE tMask );

    //  Signal
	typedef boost::function<void ( const boost::shared_ptr<util::Buffer> &buf )> ParsedCallback;
