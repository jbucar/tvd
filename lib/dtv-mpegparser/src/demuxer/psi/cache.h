...
namespace impl {
	typedef std::map<ID,util::Buffer *> CacheItemType;
	typedef std::map<size_t,CacheItemType> CacheType;
}

class Cache {
public:
	Cache( util::pool::Pool *pool );
	virtual ~Cache();
	...
	void setNetwork( size_t nIndex );
	
	util::Buffer *get( ID pid );
	void put( ID pid, BYTE *ptr, int size );

protected:
	void clear();

private:
	util::pool::Pool *_pool;
	impl::CacheType _cache;
	size_t _currentNetwork;
};
