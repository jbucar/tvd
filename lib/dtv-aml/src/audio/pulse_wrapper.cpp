...
static void context_state_cb(pa_context *c, void *userdata) {
	pa_wrapper *p = (pa_wrapper *)userdata;
    assert(c);
    assert(p);
...
pa_wrapper* pa_wrapper_new(
    ...
    if (pa_context_connect(p->context, server, (pa_context_flags_t)0, NULL) < 0) {
...
int pa_wrapper_write(pa_wrapper *p, const void*data, size_t length, int *rerror) {
    assert(p);
...
int pa_wrapper_read(pa_wrapper *p, void*data, size_t length, int *rerror) {
    assert(p);
...
int pa_wrapper_drain(pa_wrapper *p, int *rerror) {
    pa_operation *o = NULL;
...
int pa_wrapper_flush(pa_wrapper *p, int *rerror) {
    pa_operation *o = NULL;
...
pa_usec_t pa_wrapper_get_latency(pa_wrapper *p, int *rerror) {
    pa_usec_t t;
    assert(p);
...
int pa_wrapper_pause(pa_wrapper *p, int pause, int *rerror) {
    ...
    o = pa_stream_cork(p->stream, pause, success_cb, p);
    CHECK_SUCCESS_GOTO(p, rerror, o, unlock_and_fail);
...
int pa_wrapper_setvolume(pa_wrapper *p, pa_cvolume *vol, int *rerror) {
    ...
    sink_input_idx = pa_stream_get_index(p->stream);
    o = pa_context_set_sink_input_volume( p->context, sink_input_idx, vol, NULL, NULL);
    CHECK_SUCCESS_GOTO(p, rerror, o, unlock_and_fail);
