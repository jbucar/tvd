...
void* NclTransitionParser::parseTransitionBase( util::xml::dom::Node parentElement ) {
	util::xml::dom::NodeHandler *nodeHnd = getNodeHandler();

	void *parentObject = createTransitionBase( parentElement );
	...
	BOOST_FOREACH( util::xml::dom::Node element, elementNodeList ) {
		...
		if (elementTagName.compare( "importBase" ) == 0) {
			...
			void *elementObject = parseTransition( element );
...
void *NclTransitionParser::parseTransition( util::xml::dom::Node parentElement ) {
