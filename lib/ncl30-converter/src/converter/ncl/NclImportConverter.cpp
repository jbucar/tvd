...
bool NclImportConverter::addImportNCLToImportedDocumentBase( util::xml::dom::Node childObject ) {
	util::xml::dom::NodeHandler *nodeHnd = getNodeHandler();

	// apanha o alias e a localizacao do documento
	std::string docLocation = nodeHnd->attribute(childObject, "documentURI");
	NclDocumentConverter *compiler = (NclDocumentConverter*) getDocumentParser();
	bptn::NclDocument *importedDocument = compiler->importDocument( docLocation );
	bool isValid = (importedDocument != NULL);
	if (isValid) {
		std::string docAlias = nodeHnd->attribute(childObject, "alias");
