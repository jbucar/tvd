...
void *NclMetainformationConverter::createMeta( util::xml::dom::Node parentElement ) {
	std::string name, content;
	util::xml::dom::NodeHandler *nodeHnd = getNodeHandler();

	if (nodeHnd->hasAttribute(parentElement, "name")) {
		name = nodeHnd->attribute(parentElement, "name");
	} else {
		return NULL;
	}

	if (nodeHnd->hasAttribute(parentElement, "content")) {
		content = nodeHnd->attribute(parentElement, "content");
	} else {
		return NULL;
	}

	bptnm::Meta* meta = new bptnm::Meta( name, (void*) content.c_str() );
	return (void*) meta;
}

void* NclMetainformationConverter::createMetadata() {
	bptnm::Metadata *metadata = new bptnm::Metadata();
