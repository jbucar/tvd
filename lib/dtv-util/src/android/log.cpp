...
namespace impl {
static android_LogPriority converter[] = {
	/* LOG_LEVEL_UNKNOWN */ ANDROID_LOG_UNKNOWN,
	/* LOG_LEVEL_NONE	 */ ANDROID_LOG_DEFAULT,
	...
	/* LOG_LEVEL_TRACE	 */ ANDROID_LOG_VERBOSE,
	/* LOG_LEVEL_ALL	 */ ANDROID_LOG_DEFAULT
};
...
void log( LOG_LEVEL_TYPE level, const char *group, const char *category, const char *format, ... ) {
	if (level <= LOG_LEVEL_ALL) {
		android_LogPriority prio = impl::converter[level];
		va_list args;
		va_start(args, format);
		std::string formatedMsg = format_va( format, args );
		va_end( args );
		__android_log_print( prio, "dtv", "[%s::%s] %s", group, category, formatedMsg.c_str() );
	}
}
