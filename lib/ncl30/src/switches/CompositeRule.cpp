...
namespace ncl {
namespace switches {
CompositeRule::CompositeRule( const std::string &id, short someOperator ) :
		Rule( id ) {
...
CompositeRule::CompositeRule( const std::string &id, Rule* firstRule, Rule* secondRule, short someOperator ) :
		Rule( id ) {
...
std::vector<Rule*>* CompositeRule::getRules() {
...
std::string CompositeRule::toString() {
	std::string result;
	std::vector<Rule*>::iterator ruleIterator;
	Rule* rule;

	result = "(";

	for (ruleIterator = rules->begin(); ruleIterator != rules->end(); ++ruleIterator) {
