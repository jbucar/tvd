...
namespace ncl {
namespace reuse {
ReferNode::ReferNode( const std::string &id ) :
		Node( id ) {
...
ReferNode::ReferNode( const std::string &id, Entity *entity ) :
		Node( id ) {
	typeSet.insert( "ReferNode" );
	typeSet.insert( "DocumentNode" );
	typeSet.insert( "ReferEntity" );
...
const std::string &ReferNode::getInstanceType() {
	return instanceType;
}

void ReferNode::setInstanceType( const std::string &instance ) {
	if (instance != this->instanceType) {
		...
		if (this->instanceType == "new") {
			...
			if (referredNode != NULL && referredNode->instanceOf( "NodeEntity" )) {
				...
				((bptnc::NodeEntity*) referredNode)->addSameInstance( this );
			}
			...
			if (referredNode != NULL && referredNode->instanceOf( "NodeEntity" )) {
				...
				if (instance == "new") {
					((bptnc::NodeEntity*) referredNode)->removeSameInstance( this );
				}
			}
		}
	}
...
void ReferNode::setReferredEntity( Entity *entity ) {
	if (referredNode != entity) {
		...
		if (referredNode != NULL && referredNode->instanceOf( "NodeEntity" ) && instanceType != "new") {
