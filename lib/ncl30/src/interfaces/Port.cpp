...
Port::Port( const std::string &id, bptnc::Node* someNode, InterfacePoint* someInterfacePoint ) :
		InterfacePoint( id ) {
...
bptnc::Node* Port::getNode() {
	return node;
}

bptnc::Node* Port::getEndNode() {
...
std::vector<bptnc::Node*>* Port::getMapNodeNesting() {
	std::vector<bptnc::Node*>* nodeSequence;
	std::vector<bptnc::Node*>* nodeList;

	nodeSequence = new std::vector<bptnc::Node*>;
	nodeSequence->push_back( node );
	if (interfacePoint->instanceOf( "Anchor" ) || interfacePoint->instanceOf( "SwitchPort" )) {
...
void Port::setNode( bptnc::Node* someNode ) {
	node = someNode;
}
}
}
