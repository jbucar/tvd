...
{
public:
	CausalLink( const std::string &uid, bptncon::Connector *connector );
	virtual ~CausalLink() {
	}
	;
	bool containsSourceNode( bptnc::Node *node, bptnd::GenericDescriptor *descriptor );

	std::vector<Bind*> *getActionBinds();
	std::vector<Bind*> *getConditionBinds();
};
}
}
}
