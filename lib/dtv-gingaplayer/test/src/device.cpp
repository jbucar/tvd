...
TEST_F( Device, download_jpg ) {
	std::string file;
	std::string url("https://novalettres.files.wordpress.com/2011/08/tux.jpg");
	ASSERT_TRUE( device()->download(url, file) );
	ASSERT_TRUE( file.find(".jpg") != std::string::npos );
}

TEST_F( Device, download_png ) {
	std::string file;
...
TEST_F( Device, download_gif ) {
	std::string file;
	std::string url("https://upload.wikimedia.org/wikipedia/commons/e/e6/Walking_baby_tux.gif");
	ASSERT_TRUE( device()->download(url, file) );
